...
def interfaces(request, host_id):
    ...
    try:
        ...
        iface_info = None
        for iface in ifaces:
            iface_info = conn.get_iface_info(iface)
            if cmp(iface_info["type"], "ethernet") == 0 and iface_info["name"].find("GE") != -1:
                ifaces_all.append(iface_info)

        ifaces_all.sort(key=lambda data:data["name"])
