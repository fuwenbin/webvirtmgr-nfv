...
class wvmInstance(wvmConnect):
    ...
    def make_pci_attribute(self, node, attr):
        if attr is not None and len(attr) > 0:
            node.set('domain', '0x0000')
            node.set('bus', '0x' + attr.split(':')[0])
            node.set('slot', '0x' + attr.split(':')[1].split('.')[0])
            node.set('function', '0x' + attr.split(':')[1].split('.')[1])

    def make_pci_bus_attribute(self, address_item, vnic_index):
        if address_item is not None:
            address_item.set('type', 'pci')
            address_item.set('domain', '0x0000')
            address_item.set('bus', '0x00')
            address_item.set('function', '0x00')
            address_item.set('slot', '0x%02x' % (vnic_index+7))
    ...
    def change_settings(self, description, cur_memory, memory, cur_vcpu, vcpu, hd_resources = None):
        """
        Function change ram and cpu on vds.
        """
        hostdev_prefix = "<hostdev mode='subsystem' type='pci' managed='yes'>"
        hostdev_suffix = "</hostdev>"
        memory = int(memory) * 1024
        cur_memory = int(cur_memory) * 1024

        xml = self._XMLDesc(VIR_DOMAIN_XML_SECURE)
        if xml.find(hostdev_suffix) != -1:
            xml = xml[0:xml.find(hostdev_prefix)] + xml[len(hostdev_suffix) + xml.rfind(hostdev_suffix):]
        tree = ElementTree.fromstring(xml)
        ...
        devices_node = tree.find('devices')
        hostdev_attr = {'mode' : 'subsystem', 'type' : 'pci', 'managed' : 'yes' }
        if hd_resources is not None:
            for vf in hd_resources:
                element = Element('hostdev', hostdev_attr)
                source = SubElement(element, 'source')
                address = Element('address')
                self.make_pci_attribute(address, vf)
